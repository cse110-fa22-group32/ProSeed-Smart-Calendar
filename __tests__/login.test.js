...
describe("Testing Login Functionality", () => {
  beforeAll(async () => {
    await page.goto(
      "https://cse110-fa22-group32.github.io/cse110-fa22-group32/"
    );
  });
...
describe("Testing Middleground Functionality", () => {
  ...
  it("Open a calendar", async () => {
    ...
    const extraDays = await page.$$(
      ".calendar-day-block extra othermonth hidden"
    );
...
describe("Testing Calendar Functionality", () => {
  ...
  it("Add calendar event: check in main view", async () => {
    ...
    //const title = await (await text[0].getProperty('textContent')).jsonValue();
    const eventTitle = await (
      await eventsList[0].getProperty("textContent")
    ).jsonValue();
  ...
  it("Add calendar event: Check in taskbar view", async () => {
    ...
    const sidebarHeader = await (
      await (await sidebar.$(".sidebar-title")).getProperty("textContent")
    ).jsonValue();
  ...
  it("Check edit event", async () => {
    ...
    const eventTitle = await (
      await eventsList2[0].getProperty("textContent")
    ).jsonValue();
    expect(eventTitle).toBe(eventName);
