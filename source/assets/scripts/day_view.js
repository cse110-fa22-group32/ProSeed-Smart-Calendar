        ...
        if (calendarData[0].years[String(currDay[0] - 2000)].months[String(currDay[1] - 1)].days[day - 1] != null) {
          ...
          for (let task of days.Tasks) {
            todoArray.push({
              "id": task.TaskID,
              "title": task.TaskName,
              "due": task.DueDate.split(" ")[1],
            });
          }

          //add data to the new event-block
          eventArray.forEach((event) => {
            let eventBlock = document.createElement('event-block');

            eventBlock.eventData = event;
            sideBarEvent.append(eventBlock);

            let deleteButton = eventBlock.shadowRoot.querySelector("#delete");
            ...
            deleteButton.addEventListener("click", (btnEvent) => {
              ...
              let currEventsArray = calendarData[0].
              years[String(currDay[0] - 2000)].
              months[String(currDay[1] - 1)].
              days[day - 1].events;
              let currEventsLength = currEventsArray.length;

              for (let currID = 0; currID < currEventsLength; currID++) {
                let currEvent = currEventsArray[currID];
                console.log("event object id: " + currEvent.eventID);
                console.log("event id: " + event.id);
                
                // checks if event id exists in object structure
                if (currEvent.eventID == event.id) {
                  // removes event from object structure
                  calendarData[0].
                    years[String(currDay[0] - 2000)].
                    months[String(currDay[1] - 1)].
                    days[day - 1].events[currID] = null;
                  
                  //sideBarEvent.remove(btnEvent.currentTarget);

                  /* let counter = 0;
                  for (let event of eventArray) {
                    if (event.id == currEvent.eventID) {
                      delete eventArray[counter];
                      break;
                    }
          ...
          todoArray.forEach((task) => {
            let todoBlock = document.createElement('todo-block');
            todoBlock.todoData = task;
            let deleteButton = todoBlock.shadowRoot.querySelector("#delete");
            deleteButton.addEventListener("click", (btnEvent) => {
              console.log(btnEvent.currentTarget.parentNode);
              //===================
              //delete method here for todo, or have some other idea.
              //==================
            });
            sideBarTask.append(todoBlock);
          });
